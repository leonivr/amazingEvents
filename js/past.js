...
let listCategorias3 = document.querySelectorAll("label input[type='checkbox']");

listCategorias3.forEach(checkbox =>{
...
function renderSearch(){
  ...
  let resultados = pastList.filter(evento =>
...
/*filtro sin cruzar*/
listCategorias3.forEach(categoria => {
    categoria.onclick= () => {    
      let htmlResultados = "";
      let catchecked=[];
      for(cates of listCategorias3){
        if(cates.checked){
      ...
      if(catchecked.length>0){
        pastList.filter(evento => catchecked.includes(evento.category)).forEach
        ((event) =>{
